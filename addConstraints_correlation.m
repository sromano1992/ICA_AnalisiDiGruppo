...
function c_constrained = addConstraints_correlation(idx,D,CL,removeOutliers)
...
threshold = 20;    %percentage of max distance from mean
if removeOutliers
    ...
    for i=1:k
        ...
        for j=1:size(cluster_i_el,1)    
           if distance_cl_i(j) < (mean_dist-(mean_dist/100*threshold))    %removing less correlated
